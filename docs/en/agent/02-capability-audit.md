...
| Area | Status | Reality |
|---|---|---|
| Coding (166–195) | **Partial** | Navigation is real in both directions: `services/code-index.ts` maps declarations (`code.symbol.search`, `code.outline.read`, `code.file.outline.read`) and `services/code-references.ts` maps uses (`code.references.search`), classifying each occurrence as code, string, comment, import or declaration — the classification is the answer, since on this repository `redactToken` has 7 code references against 28 inside string literals. It is lexical, not type-aware, and reports `confidence: approximate` rather than guessing. Patch application and commits exist via `agent-tools-git.ts`, and `git.pull_request.create` now proposes the result under a separate forge credential. `sandbox.test` now runs inside a user/mount/net/pid namespace with a read-only root and no network — measured, not assumed: six escape probes that all succeeded before (including a write to `/tmp` confirmed on disk) are all blocked after. Read access is reduced by masking rather than eliminated, since `pivot_root` is unavailable here, and there is no CPU/memory ceiling; both are reported to the caller. Still absent: the *write* half of cross-file refactoring (an atomic multi-file rename). `repository-context-runtime.ts` and `repository-intelligence-runtime.ts` remain unreachable pure logic. |
